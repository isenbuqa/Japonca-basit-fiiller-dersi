...
const TimeWordsModule: React.FC<TimeWordsModuleProps> = ({ onBack }) => {
  const [timeWords, setTimeWords] = useState<TimeWord[]>([]);
  ...
  if (isLoading) {
    return (
      <div className="h-full bg-yellow-50 flex flex-col items-center justify-center">
        ...
        <p className="text-yellow-500 font-bold">Yükleniyor...</p>
      </div>
    );
  }
  ...
  if (timeWords.length === 0) {
     return (
       <div className="h-full bg-yellow-50 flex flex-col items-center justify-center p-6 text-center">
         <p className="text-gray-600 mb-4">Henüz hiç kelime eklenmemiş.</p>
         <button onClick={onBack} className="px-6 py-2 bg-yellow-500 text-white rounded-full font-bold">Menüye Dön</button>
       </div>
     );
  }

  const currentCard = timeWords[currentIndex];

  const handleNext = () => {
    if (currentIndex < timeWords.length - 1) {
  ...
  return (
    <div className="h-full bg-yellow-50 flex flex-col items-center">
      {/* Header */}
