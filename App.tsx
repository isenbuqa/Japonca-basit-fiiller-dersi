    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-rose-50 p-6">
         <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full text-center border border-rose-100">
            <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${info?.color.split(' ')[0]}`}>
              {info?.icon}
            </div>
            <h2 className="text-2xl font-bold text-gray-800 mb-2">{info?.title}</h2>
            <p className="text-gray-500 mb-8">Bu modülün içeriği henüz hazırlanıyor.</p>
            <button 
    ...
    <div className="min-h-screen bg-[url('https://www.transparenttextures.com/patterns/shippo.png')] bg-repeat bg-rose-50 flex flex-col overflow-y-auto">
      ...
      <div className="relative z-10 max-w-2xl mx-auto w-full p-6">
        <header className="text-center py-8">
          <span className="inline-block bg-white/80 backdrop-blur px-4 py-1.5 rounded-full text-sm font-bold text-rose-500 border border-rose-200 mb-3 shadow-sm">
            N5 Başlangıç Seviyesi
          </span>
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-800 tracking-tight mb-2">
            Japonca Ders 1
          </h1>
          <p className="text-gray-600 font-medium">Lütfen bir etkinlik seçin</p>
        </header>
        ...
        <div className="grid gap-4 pb-12">
          {MENU_ITEMS.map((item) => (
            <button
              ...
              className="group relative bg-white/90 backdrop-blur hover:bg-white border-2 border-transparent hover:border-rose-200 p-4 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 text-left flex items-center gap-4"
            >
              <div className={`p-3 rounded-xl transition-colors duration-300 ${item.color} group-hover:scale-110`}>
                {item.icon}
              </div>
              ...
              <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-rose-400 group-hover:translate-x-1 transition-all" />
            </button>
          ))}
        </div>
      </div>
